...
unsigned long long calcMin(Button& a, Button& b, Prize& p);

int main()
{
    const unsigned long long offset = 10000000000000;
    ...
    while (std::getline(infile, line))
    {
        ...
        else if (line.find("Prize:") != line.npos)
        {
            rgx.assign("X=(\\d+).*Y=(\\d+)");
            std::regex_search(line, match, rgx);
            p.x = stoi(match[1]) + offset;
            p.y = stoi(match[2]) + offset;
...
unsigned long long calcMin(Button& a, Button& b, Prize& p)
{
    ...
    long long x = ((b.y * p.x) - (b.x * p.y));
    long long y = ((a.x * p.y) - (a.y * p.x));
    ...
    if (x * a.x + y * b.x == p.x && x * a.y + y * b.y == p.y)
    {
